...
public class LittleSearchEngine {
	...
	public HashMap<String, Occurrence> loadKeyWords(String docFile)
		...
		while (sc.hasNext()) {
			...
			if (map.containsKey(word)) map.get(word).frequency++;
			else
	...
	public String getKeyWord(String word) {
		...
		for (int i = 0; i < word.length(); i++)
			if (!isAlpha(word.charAt(i))) {
				if (isNotPunc(word.charAt(i))) return null;
				if (i == word.length() - 1) {
					word = word.substring(0, i);
					break;
				} else if (!isAlpha(word.charAt(i + 1))) {
	...
	private static boolean isNotPunc(char c) {
		return !(c == '.' || c == ',' || c == '?' || c == ':' || c == ';'
				|| c == '!');
	}
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		...
		int start = 0, end = occs.size() - 2, key = occs.get(end + 1).frequency, mid = (start + end) / 2;
		ArrayList<Integer> mids = new ArrayList<Integer>();
		while (start <= end) {
			mid = (start + end) / 2;
			mids.add(mid);
			if (key > occs.get(mid).frequency) end = mid - 1;
			else if (key < occs.get(mid).frequency) {
				start = mid + 1;
				if (start > end) mid++;
			} else
				break;
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		if (keywordsIndex.containsKey(kw1) && keywordsIndex.containsKey(kw2)) {
			...
			ArrayList<Occurrence> names1 = keywordsIndex.get(kw1);
			for (int i = 0; i < Math.min(5, names1.size()); i++)
				result.add(names1.get(i));
			ArrayList<Occurrence> names2 = keywordsIndex.get(kw2);
			for (int i = 0; i < Math.min(5, names2.size()); i++)
				for (int j = 0; j < result.size(); j++)
					if (names2.get(i).frequency > result.get(j).frequency) {
						result.add(j, names2.get(i));
						break;
					} else if (names2.get(i).frequency == result.get(j).frequency
							|| j == result.size() - 1) {
						result.add(j + 1, names2.get(i));
						break;
					} else
						continue;
			for (int i = 0; i < Math.min(5, result.size()); i++)
				if (!docName.contains(result.get(i).document)) docName
						.add(result.get(i).document);
			return docName;
		}
		ArrayList<Occurrence> names = keywordsIndex.get(kw1);
		if (names == null) names = keywordsIndex.get(kw2);
		if (names == null) return null;
		for (int i = 0; i < Math.min(5, names.size()); i++)
			docName.add(names.get(i).document);
		return docName;
	}

	/*public static void main(String[] args) throws FileNotFoundException {
		// TODO Auto-generated method stub
		LittleSearchEngine t = new LittleSearchEngine();
		t.makeIndex("0", "noisewords.txt");
		for (String string : t.keywordsIndex.keySet())
			System.out.print(string + " " + t.keywordsIndex.get(string) + "\n");
		System.out.println(t.top5search("dd", "bb"));
	}*/
}